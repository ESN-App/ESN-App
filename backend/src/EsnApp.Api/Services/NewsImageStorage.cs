...
public class NewsImageStorage(IWebHostEnvironment environment)
{
    ...
    public async Task<string?> SaveAsync(IFormFile? image, CancellationToken cancellationToken)
    {
        ...
        {
            throw new InvalidOperationException("News image must be between 1 byte and 8 MB.");
        }

        if (!AllowedTypes.TryGetValue(image.ContentType, out var extension))
        {
            throw new InvalidOperationException("News image must be a JPEG, PNG, or WebP file.");
        }
        ...
        return $"/api/images/uploads/news/{fileName}";
    }
    ...
    public void Delete(string? publicPath)
    {
        ...
        if (publicPath is null ||
            !publicPath.StartsWith("/api/images/uploads/news/", StringComparison.Ordinal))
        {
            return;
        }
